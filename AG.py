...
def main():
    ...
    GerarPopulacao(tampop,populacao)
...
def converterb_d(n):
    ...
    for i in range(tam):
        if n[i] == "1":
            decimal = decimal + 2**i
    return decimal
def GerarPopulacao(tampop,populacao):
    for i in range(tampop):
        numale = random.randrange(0,31)
        populacao.append(converterd_b(numale))

    return 0
def AvaliarPop(populacao):
    candidatos = []
    for i in range(len(populacao)):
        apt = (converterb_d(populacao[i]))**2
